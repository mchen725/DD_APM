...
from networks_train import MLP, ConvNet, LeNet, AlexNet, VGG11BN, VGG11, ResNet18, ResNet18BN_AP, ResNet18_AP, ResNet50, ResNet50BN, ResNet6BN, ResNet6, ResNet18BN
...
def get_dataset(dataset, data_path, batch_size=1, subset="imagenette", args=None):
    ...
    if dataset == 'CIFAR10':
        ...
        val_transform = transforms.Compose([
            ...
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),])
        dst_train = datasets.ImageFolder(os.path.join(data_path, "train"), transform=train_transform) # no augmentation
        dst_test = datasets.ImageFolder(os.path.join(data_path, "val"), transform=val_transform)
        # dst_test = datasets.ImageFolder(os.path.join(data_path, "val"), transform=transform)
        # dst_train = datasets.ImageNet(root=data_path, split='train', transform=train_transform)
        # dst_test = datasets.ImageNet(root=data_path, split='val', transform=val_transform)
        # class_names = dst_train.classes
        class_names = [i for i in range(num_classes)]
        class_map = {x:x for x in range(num_classes)}
