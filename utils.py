...
from networks import MLP, ConvNet,ConvNetW, LeNet, AlexNet, VGG11BN, VGG11,VGG16,\
ResNet18, ResNet18BN_AP, ResNet18_AP, ResNet50, ResNet50BN, ResNet6BN, ResNet18BN, ViT, ResNet101BN
import random
...
def get_dataset(dataset, data_path, batch_size=1, subset="imagenette", args=None):
    ...
    if dataset == 'CIFAR10':
        ...
        mean = [0.485, 0.456, 0.406]
        std = [0.229, 0.224, 0.225]
        # load npz imagenet1k 64x64 dataset
        dst_img, dst_lab = [], []
        for n in range(1,11):
            print('loading batch',n)
            npz_data = np.load(os.path.join(data_path, "train_data_batch_%s"%n),allow_pickle=True)
            dst_img.append(npz_data['data'].reshape(-1,3,64,64)/255) 
            dst_lab.extend(npz_data['labels']) 
        dst_img = np.concatenate(dst_img,axis=0)
        print('now')
        npz_test_data = np.load(os.path.join(data_path, "val_data"),allow_pickle=True)
        dst_test_img = npz_test_data['data'].reshape(-1,3,64,64)/255
        dst_test_lab = npz_test_data['labels']
        for n in range(3):
            dst_img[:,n] = (dst_img[:,n]-mean[n])/std[n]
            dst_test_img[:,n] = (dst_test_img[:,n]-mean[n])/std[n]
        dst_img = torch.from_numpy(dst_img)
        dst_test_img = torch.from_numpy(dst_test_img)
        dst_lab = torch.tensor(dst_lab,dtype=torch.long)-1
        dst_test_lab = torch.tensor(dst_test_lab,dtype=torch.long)-1
        dst_train = TensorDataset(dst_img,dst_lab)
        dst_test = TensorDataset(dst_test_img,dst_test_lab)
...
def get_default_convnet_setting():
    ...
    # net_width, net_depth, net_act, net_norm, net_pooling = 512, 3, 'relu', 'instancenorm', 'avgpooling'
    return net_width, net_depth, net_act, net_norm, net_pooling



def get_network(model, channel, num_classes, im_size=(32, 32), dist=True):
    # print('manual seed:',int(time.time() * 1000) % 100000)
    ...
    elif model == 'ResNet6BN':
        net = ResNet6BN(channel=channel, num_classes=num_classes)
    elif model == 'ViT':
        net = ViT(
                image_size = im_size,
                patch_size = 4,
                ...
                emb_dropout = 0.1)
    # elif model == 'ResNet6':
    #     net = ResNet6(channel=channel, num_classes=num_classes)
...
def epoch(mode, dataloader, net, optimizer, criterion, args, aug, texture=False):
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        if args.soft_lab and mode == 'train':
...
def epoch_haba(mode, dataloader, net, optimizer, criterion, teacher_net, args, aug, texture=False):
    loss_avg, acc_avg, num_exp = 0, 0, 0
    net.to(args.device)
...
def get_eval_pool(eval_mode, model, model_eval):
    ...
    elif eval_mode == 'AA': # ablation study on network activation function
    ...
    elif eval_mode == 'A': # ablation study on network normalization layer
        model_eval_pool = ['AlexNet']
    elif eval_mode == 'V': # ablation study on network normalization layer
        model_eval_pool = ['VGG11']
    elif eval_mode == 'R': # ablation study on network normalization layer
...
class StyleTranslatorSharedDataset_mc(Dataset):
    def __init__(self, images, styles, teacher_func,args):
        self.n_c, self.ipc, _, h, w = images.shape
        self.styles = styles
        self.images = images.detach().float()
    ...
    def __getitem__(self, index):
        teacher_model = self.teacher_func(self.args)
        label = index // self.ipc
        idx = index % self.ipc
        base = self.images[label, idx, :, :, :]
        style_idx = random.randint(0, len(self.styles) - 1)
        ...
        soft_label = teacher_model(image)
        return image[0], soft_label
